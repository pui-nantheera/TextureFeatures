...
while ((r <= rmax) && (c <= cmax))
    ...
    if (mod(c + r, 2) == 0)      % up 45 direction
        ...
        elseif ((c == cmax) && (r < rmax)) && (mask(r,c)>0)
        ...
        elseif ((r > rmin) && (c < cmax))  && (mask(r,c)>0)
            output(i) = SI(r, c);
        ...
        elseif ((r < rmax) && (c > cmin))  && (mask(r,c)>0)
            %
            output(i) = SI(r, c);
    ...
    if ((r == rmax) && (c == cmax))  && (mask(r,c)>0)          % bottom right element
        output(i) = SI(r, c);
        sq_end = i;
        seq{j}=output(sq_end);
        %         position(i) = sub2ind(SI,r,c);
